...
class BinaryTree {
  ...
  preOrder() {
    ...
    return this.preOrderHelper(this.root, result);
  }
...
function insertionSort(inputArr) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    let j = i - 1;
    while (j > -1 && current.name < inputArr[j].name) {
...
function crearArray() {
  ...
  pokemonArray = insertionSort(pokemonArray); // ordeno el array
}
...
// cuando hay algo en input se empiezan a filtrar tarjetas
function rellenarTarjetasBusqueda() {
  document.getElementById("app").innerHTML = ""; // BORRA LO DE ADENTRO DEL DIV

  let datos = document.getElementById("inputData").value; //jala datos del input

  if (datos == "") {
    rellenarTarjetas(); // si no hay nada en inout que aparezcan todas
  } else {
    // busca en el array de pokemones si coincide lo que dice con los datos
    let temp_array = [];

    //buscar todas las coincidencias de las letras que se han escrito
    pokemonArray.forEach((element) => {
      if (
    ...
    //for loop para mandar los datos de las coincidencias  al html
...
// para cuando el input pierde el focus
